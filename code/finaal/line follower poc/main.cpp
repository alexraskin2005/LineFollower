...
double m1_pwm_fwd = 0,m1_pwm_bwd = 0, m2_pwm_fwd = 0,m2_pwm_bwd = 0, m1 = 0, m2 = 0, m1_raw, m2_raw; // motor PWM values and speed commands
...
float getLinePosition(uint8_t sensors[8]) {
    const int numSensors = 8;
    const float arrayWidth = 66.0f; // mm
    const float spacing = arrayWidth / (numSensors - 1); // ~9.43 mm

    // Step 1: Find the index of the maximum sensor value
    int maxIndex = 0;
    ...
    // Step 2: Ensure we have a valid triplet (i-1, i, i+1)
    if (maxIndex == 0 || maxIndex == numSensors - 1) {
        // Edge case: can't interpolate, return max sensor position
        return maxIndex * spacing;
    }

    // Step 3: Fit a parabola to the three points
    float y0 = sensors[maxIndex - 1];
    float y1 = sensors[maxIndex];
    float y2 = sensors[maxIndex + 1];

    // Parabolic vertex offset from center index
    float denominator = 2 * (y0 - 2 * y1 + y2);
    if (denominator == 0.0f) {
        return maxIndex * spacing; // Avoid division by zero
    }

    float offset = (y0 - y2) / denominator;

    // Step 4: Calculate interpolated position
    float interpolatedIndex = maxIndex + offset;
    return interpolatedIndex * spacing;
}
...
void loop() 
{
  ...
  if (current - previous >= params.cycleTime)
  {
    ...
    m1=forwardSpeed+correction;
    m2=forwardSpeed-correction;

    m1_pwm_fwd = m1 > 0 ? map(m1, 0, params.maxSpeed, 0, 255) : 0;
    m1_pwm_bwd = m1 < 0 ? map(-m1, 0, params.maxSpeed, 0, 255) : 0;

    m2_pwm_fwd = m2 > 0 ? map(m2, 0, params.maxSpeed, 0, 255) : 0;
    m2_pwm_bwd = m2 < 0 ? map(-m2, 0, params.maxSpeed, 0, 255) : 0;
      

    analogWrite(25, m1_pwm_fwd);
    analogWrite(26, m2_pwm_fwd);
